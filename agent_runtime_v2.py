...
SafeStack Agent Runtime v2

Purpose:
- run one agent command
- capture stdout as raw agent output only
- capture stderr separately
- save exact stdout to runtime/raw/<AGENT>.raw.txt
...
def run_strict_mode(agent: str, raw_path: Path) -> tuple[bool, dict[str, Any]]:
    # Kviečiame strict_mode.py kaip nepriklausomą arbitrą
...
def main() -> None:
    if len(sys.argv) < 3:
        print(json.dumps({
            "agent": "UNKNOWN",
            "status": "invalid",
            "reason": "Usage: python agent_runtime_v2.py <AGENT> <command...>"
        }, ensure_ascii=False))
        sys.exit(7)
